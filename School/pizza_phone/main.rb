...
while true # infinite loop, quit by exit commands
  ...
  Ordering.build_order_list(pizza_list) # generates list of all pizzas

  pizza_chosen = Ordering.pick_order(pizza_list, pizza_count) # creates an order. output = ordered items, total price, items ordered plus prices
  ...
  customer_info = Ordering.order_transport # gets info on delivery type
